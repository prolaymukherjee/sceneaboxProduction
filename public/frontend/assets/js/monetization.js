var _bsa = {
    init: function (format, zoneKey, segment, options) {
        ...
        if (forceUrlPreview) {
            ...
            options.testData = { ads: [Object.assign({}, previewData, { zonekey: zoneKey, statlink: previewData.link }), {}] };
        }
        ...
        if (this.isset(options) && this.isset(options.target) && document.querySelectorAll(options.target).length > 0) {
            ...
            var jsonSrc = options && options.path ? options.path : "//" + _bsa.srv() + "/ads/" + zoneKey + ".json";
            if (segment) jsonSrc = this.appendQueryString(jsonSrc, "segment", segment);
            if (options && this.isset(options.ip)) jsonSrc = this.appendQueryString(jsonSrc, "forwardedip", options.ip);
            if (options && this.isset(options.country)) jsonSrc = this.appendQueryString(jsonSrc, "country", options.country);
            if (options && this.isset(options.number_of_ads)) jsonSrc = this.appendQueryString(jsonSrc, "forcenads", options.number_of_ads);
            if (forcebanner) jsonSrc = this.appendQueryString(jsonSrc, "forcebanner", forcebanner);
            if (ignore) jsonSrc = this.appendQueryString(jsonSrc, "ignore", ignore);
            if (ignoretargeting) jsonSrc = this.appendQueryString(jsonSrc, "ignoretargeting", ignoretargeting);
            if (forwardedip) jsonSrc = this.appendQueryString(jsonSrc, "forwardedip", forwardedip);
    ...
    findInQueue: function (key) {
        for (var i = 0; i < window["_bsa_queue"].length; i++) if (window["_bsa_queue"][i][1] == key) return i;
    },
    ...
    close: function (e, t) {
        this.hide(e);
        if (this.isset(_bsa.setCookie)) _bsa.setCookie(e, "hide", this.isset(t) ? t : 1e3 * 60 * 60 * 24);
    },
...
_bsa.srvConv = function (reqHash, typeName = "unknown") {
    var types = { unknown: 0, landing: 1, "landing-next": 2, "landing-timer": 3 };
    var type = types[typeName];
    var url = "//" + _bsa.srv() + "/ads/convert/x/" + reqHash + "/t/" + type;
...
_bsa.trackConv = function () {
    ...
    if (landed) {
        _bsa.srvConv(landed, "landing-next");
    } else {
        var reqHash = this.getURLVar("_bsa_req", window.location.href);
        if (reqHash) {
            _bsa.setCookie("_bsap_landed", reqHash, 1e3 * 60 * 15);
            _bsa.srvConv(reqHash, "landing");
            setTimeout(function () {
                _bsa.srvConv(reqHash, "landing-next");
            }, 10 * 1e3);
        }
    }
};
...
_bsa.default = function (zoneKey, segment, options, ads) {
    ...
    _bsa.default.attributes = { type: "default", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    function css(colors) {
        ...
        c +=
            ...
            id +
            " a._default_{position:relative;display:flex;margin:5px;text-decoration:none}#" +
            id +
            " a._default_:hover .default-title{text-decoration:underline}#" +
            id +
            " .default-ad{position:absolute;top:0;right:0;padding:2px 3px;border-radius:2px;background:#e1e1e1;color:#9a9a9a;font-weight:600;font-size:12.5px}#" +
            id +
            " a._default_ .default-image{line-height:0;margin-right:10px}#" +
            id +
            " .default-image img{float:left;height:40px;border-radius:4px;padding:10px;margin:0}#" +
            id +
            " .default-text{line-height:1.35}#" +
            id +
            " .default-title{display:block;margin:4px 0 0 0;padding:0 0 4px;font-weight:600;font-size:16px}#" +
            id +
            " .default-description{display:block;margin-right:5%;color:#414141;font-size:14px}#" +
            id +
            " a:hover .default-description{color:#111}";
    ...
    function template(ads) {
        ...
        for (var i = 0; i < ads.length; i++) {
            c +=
                ...
                '" target="_blank" rel="noopener sponsored" class="_default_"><span class="default-image"><img src="' +
                (_bsa.isset(ads[i].base64) ? ads[i].base64 : image_type == "logo" ? ads[i].logo : _bsa.isset(ads[i].image) ? ads[i].image : ads[i].logo) +
                '" style="background: ' +
                ads[i].backgroundColor +
                ';"/></span><div class="default-text"><span class="default-title">' +
                _bsa.htmlEncode(ads[i].company) +
                '</span><span class="default-description">' +
                _bsa.htmlEncode(ads[i].description) +
                "</span></div></a>";
...
_bsa.defaultcta = function (zoneKey, segment, options, ads) {
    ...
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false;
    var image_type = _bsa.isset(options) && _bsa.isset(options.image) ? options.image : false;
    _bsa.defaultcta.attributes = { type: "default", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    function css(colors) {
        ...
        c +=
            "#" +
            id +
            "{position:relative;display:flex;box-shadow:0 0 10px #ccc;margin:0 0 40px;}#" +
            id +
            " a._defaultcta_{display:block;position:relative;text-decoration:none;padding:20px 20px 15px;}#" +
            id +
            " a._defaultcta_:nth-child(n+2){border-top:1px solid #f1f1f1;}#" +
            id +
            " a._default_ .default-title:hover{text-decoration:underline;}#" +
            id +
            " .default-ad{background:" +
            (_bsa.isset(options) && _bsa.isset(options.heading_background) ? options.heading_background : "#000000") +
            ";color:" +
            (_bsa.isset(options) && _bsa.isset(options.heading_text) ? options.heading_text : "#000000") +
            ";padding:10px;font-size:12.5px;font-weight:600;}#" +
            id +
            " a._defaultcta_ .default-image img{float:left;margin:0 20px 15px 0;height:60px;width:60px;}#" +
            id +
            " a._defaultcta_ .default-title{display:block;margin:4px 0 0 0;font-size:16px;font-weight:600;padding:0 0 4px;}#" +
            id +
            " a._defaultcta_ .default-description{display:block;font-size:14px;color:#414141;margin-right:5%;}#" +
            id +
            " a._defaultcta_ .default-cta{float:right;display:block;width:150px;padding:10px 10px;margin:10px 0 0 30px;border-radius:2px;text-align:center;background:" +
            (_bsa.isset(options) && _bsa.isset(options.cta_background) ? options.cta_background : "#000000") +
            ";color:" +
            (_bsa.isset(options) && _bsa.isset(options.cta_text) ? options.cta_text : "#000000") +
            ";}#" +
            id +
            ' a[href^="//srv.buysellads.com/"]{display:block !important;}';
        c += "</style>";
        return c;
    }
    function template(ads) {
        ...
        for (var i = 0; i < ads.length; i++) {
            c +=
                '<a href="' +
                _bsa.link(ads[i].statlink, segment, custom_domain, ads[i].timestamp) +
                '" target="_blank" rel="nofollow noopener" class="_defaultcta_"><span class="default-image"><img src="' +
                (_bsa.isset(ads[i].base64) ? ads[i].base64 : image_type == "logo" ? ads[i].logo : ads[i].image) +
                '" /></span><span class="default-cta">' +
                _bsa.htmlEncode(ads[i].callToAction) +
                '</span><span class="default-title">' +
                _bsa.htmlEncode(ads[i].company) +
                '</span><span class="default-description">' +
                _bsa.htmlEncode(ads[i].description) +
                "</span></a>";
...
_bsa.imageonly = function (zoneKey, segment, options, ads) {
    ...
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false;
    var image_type = _bsa.isset(options) && _bsa.isset(options.image) ? options.image : false;
    _bsa.imageonly.attributes = { type: "imageonly", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    function css(ads) {
        ...
        if (ads.length <= 1)
            c +=
                ...
                "px;}#" +
                id +
                " .imageonly-ad{position:absolute;right:0;bottom:0;z-index:2;background:rgba(255,255,255,.6);color:#4e4e4e;padding:2px 5px 1px 9px;font-family:arial;text-decoration:none;letter-spacing:1px;font-size:10px;text-transform:uppercase;border-radius:15px 0 0 0px;font-weight:600;}";
        else
            c +=
                "#" +
                id +
                " .imageonly-ad{background:rgba(255,255,255,.6);border:1px solid #eaeaea;color:#4e4e4e;width:100%;text-align:center;margin:15px 0 0 0;padding:5px 12px;font-family:arial;text-decoration:none;letter-spacing:1px;font-size:12px;text-transform:capitalize;border-radius:3px;font-weight:500;}";
        c +=
            "#" +
            id +
            "{position:relative;display:flex;flex-wrap:wrap;}#" +
            id +
            " a._imageonly_{display:block;position:relative;margin:0;padding:0;text-decoration:none;height:" +
            ads[0].height +
            "px;width:" +
            ads[0].width +
            "px;}#" +
            id +
            " a._imageonly_ .imageonly-image img{margin:0;padding:0;}";
        if (_bsa.isset(ads[0].custom_css)) c += ads[0].custom_css;
        c += "</style>";
        return c;
    }
        ...
        for (var i = 0; i < ads.length; i++) {
            if (_bsa.isset(ads[i].snippet)) c += '<span class="_imageonly_ ad' + i + " " + ((i + 1) % 2 ? "odd" : "even") + '">' + ads[i].snippet + "</span>";
                ...
                c +=
                    '<a href="' +
                    _bsa.link(ads[i].statlink, segment, custom_domain, ads[i].timestamp) +
                    '" target="_blank" rel="noopener sponsored" class="_imageonly_ ad' +
                    i +
                    " " +
                    ((i + 1) % 2 ? "odd" : "even") +
                    '"><img src="' +
                    (_bsa.isset(ads[i].base64) ? ads[i].base64 : ads[i].image) +
                    '" alt="' +
                    _bsa.htmlEncode(ads[i].alt) +
                    '" width="' +
                    ads[i].width +
                    '" height="' +
                    ads[i].height +
...
_bsa.fancybar = function (zoneKey, segment, options, ads) {
    var disable_css = _bsa.isset(options) && _bsa.isset(options.disable_css) ? options.disable_css : false;
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false;
    _bsa.fancybar.attributes = { type: "fancybar", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    if (cryptobar.indexOf(zoneKey) > 0) show_ads_via = true;
    ...
    function css(colors, show_ads_via) {
        ...
        style +=
            "#" +
            id +
            "._bsa_show ._bsa_fancybar{top:0}#" +
            id +
            " ._bsa_fancybar{position:fixed;top:-50%;right:0;left:0;z-index:100001;margin:0 auto;background-color:" +
            backgroundColor +
            ';box-shadow:0 1px 10px hsla(0,0%,0%,.25);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",Helvetica,Arial,sans-serif;-webkit-transition:top .5s ease-in-out;-moz-transition:top .5s ease-in-out;-o-transition:top .5s ease-in-out;transition:top .5s ease-in-out;backface-visibility:hidden}#' +
            id +
            " .fancybar-wrap{display:flex;box-sizing:border-box;padding:14px 20px;text-align:left;text-decoration:none;justify-content:space-between}#" +
            id +
            " a:hover{background:" +
            backgroundHoverColor +
            "}#" +
            id +
            " .fancybar-left{display:flex;align-items:center}#" +
            id +
            " .fancybar-right{display:flex;align-items:center}#" +
            id +
            " a .fancybar-cta{box-sizing:border-box;padding:8px 12px;border-radius:4px;background-color:" +
            ctaBackgroundColor +
            ";color:" +
            callToActionTextColor +
            ";text-transform:uppercase;white-space:nowrap;letter-spacing:1px;line-height:1;font-weight:600;font-size:12px}._bsa_fancybar a:hover .fancybar-cta{background:" +
            ctaBackgroundHoverColor +
            ";color:" +
            callToActionTextColorHover +
            "}#" +
            id +
            " .fancybar-logo{line-height:0}#" +
            id +
            " .fancybar-logo img{margin-right:20px;width:100px;background-color:transparent}#" +
            id +
            " a .fancybar-text{display:flex;flex-direction:column;margin-right:20px;color:" +
            textColor +
            ";font-size:16px;line-height:1.5}#" +
            id +
            " a:hover .fancybar-text{color:" +
            textColorHover +
            "}#" +
            id +
            " .fancybar-disclaimer{font-style:italic;font-size:12px;color:" +
            textColor +
            "cc;letter-spacing:.5px;line-height:1.5}#" +
            id +
            " .fancybar-footer{position:absolute;right:20px;bottom:4px;border-top:0;border-top-left-radius:3px;color:" +
            textColor +
            ";text-decoration:none;text-transform:uppercase;letter-spacing:1px;line-height:1;font-weight:600;font-size:8px}#" +
            id +
            " .fancybar-close{color:" +
            textColor +
            "cc;cursor:pointer}#" +
            id +
            " .fancybar-close:hover{color:" +
            textColor +
            "}#" +
            id +
            " .fancybar-via{color:" +
            textColor +
            "cc;font-size:8px;text-decoration:none}#" +
            id +
            " .fancybar-via:hover{color:" +
            textColor +
            "}@media only screen and (min-width:480px) and (max-width:759px){#" +
            id +
            " a .fancybar-text{font-size:14px;margin-right:0}#" +
            id +
            " .fancybar-right{display:none}#" +
            id +
            " .fancybar-disclaimer{font-size:10px}}@media only screen and (min-width:320px) and (max-width:479px){#" +
            id +
            " a .fancybar-text{font-size:12px;margin-right:0}#" +
            id +
            " .fancybar-right{display:none}#" +
            id +
            " .fancybar-logo img{margin-right:10px;width:80px}#" +
            id +
            " .fancybar-disclaimer{font-size:10px}}";
    ...
    function template(ads) {
        var a = ads[0],
            ...
            c = "";
        c +=
            '<div class="_bsa_fancybar">' +
            '<a href="' +
            statlink +
            '" class="fancybar-wrap" target="_blank" rel="noopener sponsored">' +
            '<span class="fancybar-left"><span class="fancybar-logo"><img src="' +
            logo +
            '" /></span>' +
            '<span class="fancybar-text">' +
            '<span class="fancybar-desc">' +
            _bsa.htmlEncode(description) +
            "</span>";
        if (show_ads_via && tagline.length > 30) c += '<span class="fancybar-disclaimer" title="' + _bsa.htmlEncode(tagline) + '">' + _bsa.htmlEncode(tagline) + "</span>";
        c += "</span></span></span>" + '<span class="fancybar-right"><span class="fancybar-cta">' + _bsa.htmlEncode(callToAction) + "</span></span></a>";
        c += '<div class="fancybar-footer"><span class="fancybar-close" onclick="_bsa.close(\'' + _bsa.fancybar.elID(options) + '\')">Hide Ad</span> &bull; <a class="fancybar-via" href="' + adViaLink + '">Ad via BuySellAds</a></div>';
...
_bsa.flexbar = function (zoneKey, segment, options, ads) {
    ...
    _bsa.flexbar.attributes = { type: "flexbar", key: zoneKey, segment: segment, options: options, ads: ads };
...
_bsa.hellobar = function (zoneKey, segment, options, ads) {
    var disable_css = _bsa.isset(options) && _bsa.isset(options.disable_css) ? options.disable_css : false;
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false;
    _bsa.hellobar.attributes = { type: "hellobar", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    function css() {
        ...
        style +=
            "#" +
            id +
            '{z-index:100;position:fixed;bottom:2em;left:50%;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif;font-size:12px;transition:transform .5s ease-in-out,opacity .2s ease-in-out}#' +
            id +
            ",#" +
            id +
            "._bsa_hide{transform:translate3d(-50%,20vh,0);opacity:0}#" +
            id +
            "._bsa_show{transform:translate3d(-50%,0,0);opacity:1}#" +
            id +
            " section{border-radius:.25rem;box-shadow:0 1px 4px 1px hsla(0,0%,0%,0.05)}#" +
            id +
            " a{display:flex;justify-content:center;align-items:center;padding:1ex 2ch;max-width:600px;text-decoration:none}#" +
            id +
            " img{margin-right:2ch}#" +
            id +
            " .hello-desc{margin-top:.1ex;font-size:1em;font-weight:600;line-height:1.4}#" +
            id +
            " .hello-hide{position:absolute;padding:1ex 1ch;font-size:.5em;font-weight:600;cursor:pointer;letter-spacing:.05ch;text-transform:uppercase;border-top-right-radius:.2rem;border-bottom-left-radius:.2rem;inset:0 0 auto auto}@media only screen and (min-width:320px) and (max-width:759px){#" +
            id +
            "{bottom:0;left:0;font-size:11px}#" +
            id +
            ",#" +
            id +
            "._bsa_hide{transform:translate3d(0,20vh,0)}#" +
            id +
            "._bsa_show{transform:translate3d(0,0,0)}#" +
            id +
            " section{width:100vw}#" +
            id +
            " a{max-width:inherit}}";
    ...
    function template(ads) {
        ...
        c += '<div class="hello-desc">Supported by ' + ad.company + " â€” " + ad.description + "</div>";
        c += "</a>";
        c += '<span class="hello-hide" style="background-color: ' + ad.ctaBackgroundColor + "; color: " + ad.ctaTextColor + '" onclick="_bsa.close(\'' + _bsa.hellobar.elID(options) + "')\">Hide</span>";
        c += "</section>";
...
_bsa.recommended = function (zoneKey, segment, options, ads) {
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false,
        single = [],
        c = [];
    _bsa.recommended.attributes = { type: "recommended", key: zoneKey, segment: segment, options: options, ads: ads };
...
_bsa.list = function (zoneKey, segment, options, ads) {
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false,
        single = [],
        c = [];
    _bsa.list.attributes = { type: "list", key: zoneKey, segment: segment, options: options, ads: ads };
...
_bsa.stickybox = function (zoneKey, segment, options, ads) {
    ...
    _bsa.stickybox.attributes = { type: "stickybox", key: zoneKey, segment: segment, options: options, ads: ads };
...
_bsa.custom = function (zoneKey, segment, options, ads) {
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false,
        single = [];
    _bsa.custom.attributes = { type: "custom", key: zoneKey, segment: segment, options: options, ads: ads };
        ...
        for (var i = 0; i < ads.length; i++) {
            ...
            Object.keys(ads[i]).map(function (key, index) {
                ...
                if (key === "statlink") single[i] = single[i].replace(new RegExp("##" + key + "##", "g"), _bsa.link(ads[i][key], segment, custom_domain, ads[i].timestamp));
                    ...
                    single[i] = single[i].replace(new RegExp("##" + key + "##", "g"), function () {
...
_bsa.hovercard = function (zoneKey, segment, options, ads) {
    ...
    var custom_domain = _bsa.isset(options) && _bsa.isset(options.custom_domain) ? options.custom_domain : false;
    var image_type = _bsa.isset(options) && _bsa.isset(options.image) ? options.image : false;
    _bsa.hovercard.attributes = { type: "hovercard", key: zoneKey, segment: segment, options: options, ads: ads };
    ...
    function css(colors) {
        ...
        c +=
            "@-webkit-keyframes hover-swing{20%{-webkit-transform:rotate3d(0,0,1,10deg);transform:rotate3d(0,0,1,10deg)}40%{-webkit-transform:rotate3d(0,0,1,-7deg);transform:rotate3d(0,0,1,-7deg)}60%{-webkit-transform:rotate3d(0,0,1,5deg);transform:rotate3d(0,0,1,5deg)}80%{-webkit-transform:rotate3d(0,0,1,-5deg);transform:rotate3d(0,0,1,-5deg)}to{-webkit-transform:rotate3d(0,0,1,0);transform:rotate3d(0,0,1,0)}}@keyframes hover-swing{20%{-webkit-transform:rotate3d(0,0,1,10deg);transform:rotate3d(0,0,1,10deg)}40%{-webkit-transform:rotate3d(0,0,1,-7deg);transform:rotate3d(0,0,1,-7deg)}60%{-webkit-transform:rotate3d(0,0,1,5deg);transform:rotate3d(0,0,1,5deg)}80%{-webkit-transform:rotate3d(0,0,1,-5deg);transform:rotate3d(0,0,1,-5deg)}to{-webkit-transform:rotate3d(0,0,1,0);transform:rotate3d(0,0,1,0)}}#" +
            id +
            '{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif}#' +
            id +
            " .hover-section{position:relative;left:14px}#" +
            id +
            " .hover-banner,#" +
            id +
            " .hover-subset{position:relative;top:14px;left:-14px}#" +
            id +
            " .hover-section{display:flex;flex-direction:column;max-width:250px;border-radius:8px;background-color:#201b2a;text-decoration:none}#" +
            id +
            " .hover-banner:before{transition:all .2s ease-in-out}#" +
            id +
            ' .hover-banner:before{position:absolute;bottom:0;left:0;width:0;height:0;border-width:0;border-top-right-radius:0;background-color:hsla(0,0%,100%,0);content:""}#' +
            id +
            " .hover-banner:hover{-webkit-transform-origin:top center;transform-origin:top center;-webkit-animation-name:hover-swing;animation-name:hover-swing;-webkit-animation-duration:1s;animation-duration:1s;-webkit-animation-fill-mode:both;animation-fill-mode:both}#" +
            id +
            " .hover-banner:hover:before{width:100%;height:100%;border-width:60px;border-top-right-radius:100%;background-color:hsla(0,0%,100%,.08)}#" +
            id +
            " .hover-ad{position:absolute;top:0;right:0;padding:3px 8px;border-top-right-radius:8px;border-bottom-left-radius:8px;text-transform:uppercase;font-weight:600;font-size:9px}#" +
            id +
            " .hover-banner{display:flex;margin-bottom:14px;height:140px;border-radius:8px;box-shadow:inset 0 0 0 1px hsla(0,0%,0%,.1);justify-content:center;align-items:center}#" +
            id +
            " .hover-logo{width:125px;height:50px}#" +
            id +
            " .hover-subset{display:flex}#" +
            id +
            " .hover-icon{box-sizing:content-box;margin-right:10px;padding:4px;height:20px;border:solid 5px #201b2a;border-radius:50%;min-width:auto}#" +
            id +
            " .hover-description{margin-bottom:30px;color:#ddd;font-size:14px;line-height:1.4;text-align:left}";
        if (_bsa.isset(ads[0].custom_css)) c += ads[0].custom_css;
        c += "</style>";
        return c;
    }
...
_bsa.iab = function (zoneKey, segment, options, ads) {
    ...
    _bsa.iab.attributes = { type: "iab", key: zoneKey, segment: segment, options: options, ads: ads };
    function css(colors) {
        var c = colors[0],
            ...
            backgroundHoverColor =
                _bsa.isset(c) && _bsa.isHex(c.barcolorHover)
                    ? c.barcolorHover
                    : _bsa.isset(c.backgroundHoverColor)
                    ? c.backgroundHoverColor
                    : _bsa.isset(c.backgroundHoverColor)
                    ? c.backgroundHoverColor
                    : _bsa.testData.ads[0].backgroundHoverColor,
            ...
            ctaBackgroundColor =
                _bsa.isset(c) && _bsa.isHex(c.callToActionColor)
                    ? c.callToActionColor
                    : _bsa.isset(c.callToActionBackgroundColor)
                    ? c.callToActionBackgroundColor
                    : _bsa.isset(c.ctaBackgroundColor)
                    ? c.ctaBackgroundColor
                    : _bsa.testData.ads[0].ctaBackgroundColor,
        ...
        style +=
            ...
            " .z_" +
            zoneKey +
            ' ._bsa_iab{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif;text-decoration:none;box-sizing:border-box;display:flex;position:relative;background-color:' +
            backgroundColor +
            ";border:solid 1px hsla(0,0%,0%,.2)}#" +
            id +
            " .z_" +
            zoneKey +
            " .iab-image{max-height:50px}#" +
            id +
            " .z_" +
            zoneKey +
            " .iab-details{color:" +
            ...
            " .z_" +
            zoneKey +
            " .iab-company{text-transform:uppercase;letter-spacing:1px;font-size:11px}#" +
            id +
            " .z_" +
            zoneKey +
            " .iab-desc{letter-spacing:1px;font-weight:300;font-size:14px;line-height:1.4}#" +
            id +
            " .z_" +
            zoneKey +
            " .iab-cta{padding:10px 14px;border-radius:3px;text-transform:uppercase;letter-spacing:1px;font-weight:400;font-size:12px;color:" +
            callToActionTextColor +
            ";background-color:" +
            ctaBackgroundColor +
            "}#" +
            ...
            " .z_" +
            zoneKey +
            " .size-rectangle{width:300px;height:250px;flex-direction:column;align-items:start;padding:1em;justify-content:center}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-300x250 .iab-company,#" +
            id +
            " .z_" +
            zoneKey +
            " .size-rectangle .iab-company{position:absolute;bottom:1.2em}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-300x250 .iab-cta,#" +
            id +
            " .z_" +
            zoneKey +
            " .size-rectangle .iab-cta{margin-bottom:2em}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-300x250>*+*,#" +
            ...
            " .z_" +
            zoneKey +
            " .size-leaderboard{justify-content:start;align-items:center;padding:1em;width:728px;height:90px}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-728x90 .iab-cta,#" +
            ...
            " .z_" +
            zoneKey +
            " .size-leaderboard .iab-cta{white-space:nowrap}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-728x90>*+*,#" +
            ...
            " .z_" +
            zoneKey +
            " .size-leaderboard>*+*{margin-left:1em}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-160x600{flex-direction:column;align-items:start;justify-content:center;width:160px;height:600px;padding:10px}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-160x600 .iab-company{position:absolute;line-height:1.4;bottom:1em}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-160x600 .iab-desc{margin-top:1em}#" +
            id +
            " .z_" +
            zoneKey +
            " .size-160x600>*+*{margin-top:1em}";
        if (_bsa.isset(ads[0].custom_css)) style += ads[0].custom_css;
        style += "</style>";
        return style;
    }
    function template(ads) {
        ...
        for (var i = 0; i < 1; i++) {
            c +=
                ...
                zoneKey +
                '"><a href="' +
                _bsa.link(ads[i].statlink, segment, custom_domain, ads[i].timestamp, options.clicktag) +
                '" target="_blank" rel="noopener sponsored" class="_bsa_iab size-' +
                _bsa.iab.iabformat(options) +
                '"><img class="iab-image" height="50" width="125" src="' +
                (_bsa.iab.iabformat(options) == "rectangle" ? ads[i].logo : ads[i].logo) +
                '" /><span class="iab-details"><div class="iab-company">Sponsored by <strong>' +
                _bsa.htmlEncode(ads[i].company) +
                '</strong></div><div class="iab-desc">' +
                _bsa.htmlEncode(ads[i].description) +
                '</div></span><span class="iab-cta">' +
                _bsa.htmlEncode(ads[i].callToAction) +
                "</span></a></div>";
            c += _bsa.pixel(ads[i].pixel, ads[i].timestamp);
        }
